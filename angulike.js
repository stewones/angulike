 ...
 */
(function() {
    angular.module('angulike', []).directive('fbLike', ['$window', 'setting', function($window, setting) {
        return {
            restrict: 'A',
            scope: {
                fbLike: '=?'
            ...
            link: function(scope, element, attrs) {
                if (!$window.FB) {
                    ...
                    $.getScript('//connect.facebook.net/pt_BR/sdk.js', function() {
                        $window.FB.init({
                            appId: setting.facebook.appId, //@todo make me an attribute
                            xfbml: true,
                            version: 'v2.0'
                        });
                        renderLikeButton();
                ...
                scope.$watch('fbLike', function(nv, ov) {
                    if (nv != ov) {
                        renderLikeButton();
                    }
                });
    ...
    }]).directive('googlePlus', ['$window', '$rootScope', function($window, $rootScope) {
        return {
            restrict: 'A',
            scope: {
                googlePlus: '=?'
            ...
            link: function(scope, element, attrs) {
                ...
                scope.$watch('googlePlus', function(nv, ov) {
                    if (nv != ov) {
                        renderPlusButton();
                    }
                });
    ...
    }]).directive('tweet', ['$window', '$location', '$rootScope',
        function($window, $location, $rootScope) {
            return {
                ...
                link: function(scope, element, attrs) {
                    ...
                    scope.$watch('tweetUrl+tweet', function(nv, ov) {
                        if (nv != ov) {
                            renderTweetButton();
                        }
                    });
